...
## Purpose
Global CSS files loaded in `main.tsx` via `import './styles/global.css'`. Provides design tokens (CSS custom properties), CSS reset, typography classes, texture overlays, and transition utilities. No Tailwind -- all styling is raw CSS with PostCSS (nested, autoprefixer, cssnano).

## Key Files
| File | Description |
|------|-------------|
| `variables.css` | Design tokens as CSS custom properties on `:root`. Defines the brand color palette: `--burgundy` (#721311), `--deep-red` (#730707), `--panel-red` (#870e0b), `--wine` (#4f0d0c), `--bright-red` (#e3302c), `--gold` (#ffd400), `--cream` (#f0f0f0), `--ink` (#101828), `--muted` (#4a5565). Also includes AI page colors, gold gradient, and home drum animation variables. Sets base font-family to Inter |
...
| `global.css` | Import hub: imports variables.css, reset.css, texture.css, typography.css, and transitions.css in order |
