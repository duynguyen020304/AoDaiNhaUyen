...
public interface IAdminChatPersistence
{
  ...
  Task<ChatMessage> AddMessageAsync(Guid threadId, string role, string content,
    string? toolCallsJson, string? structuredPayloadJson, CancellationToken ct);
  Task<List<ChatMessage>> GetMessagesAsync(Guid threadId, CancellationToken ct);
  Task DeleteThreadAsync(Guid threadId, Guid adminUserId, CancellationToken ct);
}
