...
## Purpose
ASP.NET Core API controllers. Each controller is thin -- it handles HTTP concerns (routing, request parsing, response formatting) and delegates all business logic to Application-layer services via DI.
...
| `AuthController.cs` | `api/auth` | Mixed | Full authentication: register, login, Google/Facebook OAuth, refresh, logout, email verification (GET redirect), forgot-password, reset-password, /me (authorized). Manages HttpOnly cookies for tokens |
| `CategoriesController.cs` | `api/v1/categories` | No | Category listing: GET returns flat list, GET header returns tree structure for navigation |
...
- Request records are defined as nested sealed records inside controllers (e.g., `AuthController.RegisterRequest`)
...
- ChatController is unique in supporting both authenticated and anonymous access via guest cookie
