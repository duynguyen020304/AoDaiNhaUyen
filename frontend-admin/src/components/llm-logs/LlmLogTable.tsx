  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-dashed bg-white p-10 text-center">
        <p className="text-sm font-medium text-ink">Không có log nào khớp bộ lọc.</p>
        <p className="mt-1 text-xs text-muted-foreground">Thử mở rộng thời gian hoặc bỏ bớt điều kiện lọc.</p>
      </div>
    )
  }
          ...
          <TableBody>
            {items.map((item) => (
              ...
              >
                ...
                <TableCell className="text-xs text-muted-foreground">{item.provider}{item.model ? ` / ${item.model}` : ''}</TableCell>
                <TableCell className="text-xs">
                  <div className="font-medium text-ink">{item.operation}</div>
                  <div className="text-muted-foreground">{item.toolName || '—'}</div>
                </TableCell>
                <TableCell>{item.latencyMs ? `${item.latencyMs}ms` : '—'}</TableCell>
                <TableCell>{item.totalTokens ?? '—'}</TableCell>
                <TableCell className="font-mono text-xs text-muted-foreground">{item.requestId.slice(0, 10)}…</TableCell>
