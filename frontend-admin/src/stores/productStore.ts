...
export const useProductStore = create<ProductState>((set, get) => ({
  ...
  updateVariant: async (productId: string, variantId: string, data: UpdateVariantRequest) => {
    set({ error: null })
    try {
      const product = await adminApi.updateVariant(productId, variantId, data)
      await get().fetchProducts()
      invalidateAdminDashboardQueries()
      return product
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Không thể cập nhật biến thể.'
      set({ error: message })
      throw err
    }
  },
